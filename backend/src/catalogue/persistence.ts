...
export async function insertModel(
  ...
  await transaction.insertInto('catalogue_models').values(model).executeTakeFirstOrThrow();
}
...
export async function insertDraftAsset(
  transaction: Transaction<CatalogueDatabaseSchema>,
  ...
  await transaction.insertInto('catalogue_assets').values(asset).executeTakeFirstOrThrow();
  await transaction
